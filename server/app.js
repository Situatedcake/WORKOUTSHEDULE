...
import { buildTrainingPlan } from "../src/shared/trainingPlanBuilder.js";
import { DEFAULT_WORKOUT_TIME } from "../src/shared/workoutSchedule.js";
...
export function createServerApp({ userRepository, databaseProvider }) {
  ...
  app.put("/api/users/:id/training-plan", async (request, response) => {
    const { workoutsPerWeek, focusKey, sessionSelections = [] } = request.body ?? {};
    ...
    async (request, response) => {
      ...
      try {
        const user = await userRepository.completeWorkout(request.params.id, {
          ...
          weightKg:
            weightKg == null || weightKg === "" ? null : Number(weightKg) || null,
          burnedCalories:
