...
const TIME_RANGE_OPTIONS = [
  {
    ...
    label: "7 дней",
    periodDays: 7,
    scopeLabel: "за последние 7 дней",
  },
  {
    key: "30",
    label: "30 дней",
    periodDays: 30,
    scopeLabel: "за последние 30 дней",
  },
  {
    key: "all",
    label: "Все время",
    periodDays: null,
    scopeLabel: "за всё время",
  },
];
  ...
  return (
    <div className="flex items-start justify-between gap-4">
      <div className="space-y-1">
        ...
        {description ? (
          <p className="text-sm leading-6 text-[#8E97A8]">{description}</p>
        ) : null}
      </div>
      {action}
    </div>
...
export default function StatisticPage() {
  ...
  const planAdaptationBreakdown = getTrainingPlanAdaptationBreakdown(trainingPlan);
      ...
      <section className="mx-auto flex w-full max-w-md flex-col gap-5 rounded-[28px] border border-[#2A3140] bg-[#12151C] p-5">
            ...
            {adaptationSummary.length > 0 ? (
              <div className="mt-2 flex flex-col gap-2">
                {adaptationSummary.slice(0, 2).map((item) => (
                  <p key={item} className="text-sm leading-6 text-[#9BD4C7]">
                    {item}
                  </p>
                ))}
              </div>
            ) : (
              <p className="mt-2 text-sm leading-6 text-[#9BD4C7]">
                Следующий цикл уже подстроен под твою историю выполнения и
                рабочие веса.
              </p>
            )}
          </div>
        ...
        {statsError ? (
          <div className="rounded-2xl border border-[#5E4B1D] bg-[#2E2510] px-4 py-3 text-sm leading-6 text-[#F3D9A1]">
            Показываем локальную статистику, пока расширенные данные с сервера
            недоступны.
          </div>
        ) : null}
        ...
        <div className="rounded-[24px] bg-[#0B0E15] px-5 py-5">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="shrink-0 rounded-2xl bg-[#12151C] px-4 py-3 text-right">
              <p className="text-xs uppercase tracking-[0.14em] text-[#667085]">
                В периоде
              </p>
              ...
              <p className="mt-1 text-xs text-[#8E97A8]">полезных сессий</p>
            </div>
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            {statusItems.map((item) => {
              ...
              return (
                <div key={item.key} className="rounded-2xl bg-[#12151C] px-4 py-3">
                  <p className="text-sm text-[#8E97A8]">{meta.label}</p>
                  <div className="mt-2 flex items-end justify-between gap-3">
                    <p className={`text-2xl font-medium ${meta.textClassName}`}>
                      {item.count}
                    </p>
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap gap-2">
                {ADAPTATION_LEGEND.map((item) => (
                  <div
                    key={item.key}
                    className="flex items-center gap-2 rounded-full bg-[#12151C] px-3 py-1.5 text-xs text-[#B9C1CF]"
                  >
              ...
              {latestAdaptationPoint ? (
                <div className="rounded-2xl bg-[#12151C] px-4 py-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-xs uppercase tracking-[0.14em] text-[#8E97A8]">
                      {getAdaptationTriggerLabel(latestAdaptationPoint.trigger)}
                    </span>
                  </div>
                  <p className="mt-2 text-sm leading-6 text-[#8E97A8]">
                    {latestAdaptationPoint.changedExercisesCount} изменений в
                    упражнениях и объёме на последнем обновлении.
                  </p>
                </div>
              ) : (
                <p className="text-sm leading-6 text-[#8E97A8]">
                  Как только план начнёт обновляться, здесь появится история его
                  изменений по циклам.
                </p>
              )}
            </div>
          ...
          footer={
            latestLoadPoint ? (
              <div className="grid grid-cols-3 gap-3">
                ...
                <MetricCard
                  label="Калории"
                  value={latestLoadPoint.calories ?? 0}
                />
                <MetricCard
                  label="Объём веса"
          ...
          footer={
            latestProgressPoint ? (
              <div className="rounded-2xl bg-[#12151C] px-4 py-3">
                ...
                <p className="mt-2 text-sm leading-6 text-[#8E97A8]">
                  {formatProgressValue(latestProgressPoint)} на{" "}
                  {latestProgressPoint.label}
                </p>
              </div>
            ) : (
              <p className="text-sm leading-6 text-[#8E97A8]">
                Как только в истории накопятся веса по подходам или данные по весу
                тела, здесь появится наглядный тренд.
              </p>
            )
          }
        >
          ...
          <div className="rounded-[24px] bg-[#0B0E15] px-5 py-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              {planAdaptationBreakdown.map((item) => (
                <div
                  key={item.key}
                  className="rounded-2xl bg-[#12151C] px-4 py-3"
                >
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <p className="text-sm text-[#8E97A8]">{item.meta.label}</p>
                  </div>
                  <p className={`mt-2 text-2xl font-medium ${item.meta.textClassName}`}>
                    {item.count}
                  </p>
                </div>
              ))}
            ...
            {planAdaptationHighlights.length > 0 ? (
                ...
                {planAdaptationHighlights.map((item) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <VolumeTrendIcon
                            ...
                            className={`h-4 w-4 ${item.meta.textClassName}`}
                          />
                          <p className={`text-sm font-medium ${item.meta.textClassName}`}>
                            {item.reasonTitle}
                          </p>
                        </div>
                      ...
                      <span
                        className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] uppercase tracking-[0.14em] ${item.meta.badgeClassName}`}
                      >
                        {item.meta.label}
                      </span>
                    </div>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {item.chips.map((chip) => (
                        <span
                          ...
                          className={`rounded-full px-2.5 py-1 text-[10px] uppercase tracking-[0.14em] ${item.meta.badgeClassName}`}
                        >
                          {chip}
                        </span>
                      ))}
              ...
              <p className="mt-4 text-sm leading-6 text-[#8E97A8]">
                Пока активная программа идёт по базовому объёму. Как только
                появятся прогресс по весам, плато или ручные правки, здесь будет
                видно, что именно поменялось.
              </p>
            )}
          </div>
        ) : null}
        ...
        <div className="rounded-[24px] bg-[#0B0E15] px-5 py-5">
          ...
          {stats.topExercises.length > 0 ? (
            <div className="mt-4 flex flex-col gap-3">
              {stats.topExercises.map((exercise, index) => (
                <div
                  ...
                  className="rounded-2xl bg-[#12151C] px-4 py-4"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-white">
                    ...
                    <div className="rounded-full bg-[#1C2431] px-3 py-1 text-xs text-[#B9C1CF]">
                      #{index + 1}
                    </div>
                  </div>
            ...
            <p className="mt-3 text-sm leading-6 text-[#8E97A8]">
              Как только начнёшь записывать веса по подходам, здесь появятся
              упражнения-лидеры и станет видно, где идёт лучший рост.
            </p>
          )}
        </div>
        ...
        <div className="rounded-[24px] bg-[#0B0E15] px-5 py-5">
          ...
          {stats.recentWorkouts.length > 0 ? (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                      <div className="rounded-xl bg-[#0F131B] px-2 py-2">
            ...
            <p className="mt-3 text-sm leading-6 text-[#8E97A8]">
              После первой завершённой тренировки здесь появится история по
              сессиям и статусам выполнения.
            </p>
          )}
        </div>
      </section>
