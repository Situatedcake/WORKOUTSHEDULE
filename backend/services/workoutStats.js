import {
  buildTrainingPlanVolumeBreakdown,
  normalizeTrainingPlanAdaptationHistory,
} from "./trainingPlanAdaptationHistory.js";
import {
...
function calculateWorkoutTrackedWeight(workout = {}) {
  return getTrackedWeightValues(workout).reduce((total, value) => total + value, 0);
}

function getLatestRecordedWeight(workoutHistory = []) {
...
function getPersonalBestSetWeight(workoutHistory = []) {
...
function getAverageWorkoutDurationMinutes(workoutHistory = []) {
  ...
  return Math.round(totalDurationSeconds / workoutHistory.length / 60);
}
...
function getWorkoutStreakDays(workoutHistory = [], todayDateKey) {
  ...
  const uniqueDates = Array.from(
    new Set(
      productiveWorkouts.map((workout) => normalizeDateKey(workout.date)).filter(Boolean),
    ),
...
function getTopExercisesByTrackedWeight(workoutHistory = [], limit = 3) {
...
function buildAdaptationTrend(
  ...
  const normalizedHistory = normalizeTrainingPlanAdaptationHistory(
    trainingPlanAdaptationHistory,
  );
  const sourceHistory =
...
export function buildWorkoutStatsPayload({
  workoutHistory = [],
  todayDateKey,
  scheduledWorkouts = [],
  trainingPlanAdaptationHistory = [],
  ...
  const normalizedWorkoutHistory = normalizeWorkoutHistory(workoutHistory);
  const currentDateKey =
    normalizeDateKey(todayDateKey) || normalizeDateKey(new Date().toISOString());
  const filteredWorkoutHistory = periodDays
    ? normalizedWorkoutHistory.filter((workout) =>
    ...
    : normalizedWorkoutHistory;
  const currentMonth = currentDateKey.slice(0, 7);
  const completedThisMonth = filteredWorkoutHistory.filter((workout) => {
    const workoutDateKey = normalizeDateKey(workout.date);

    return (
      workoutDateKey.startsWith(currentMonth) &&
      isProductiveWorkoutStatus(workout.status)
    );
  });
  ...
  const completionRate = totalLogged
    ? Math.round(
        ((statusBreakdown.completed + statusBreakdown.partial) / totalLogged) * 100,
      )
    : 0;
  const normalizedAdaptationHistory = normalizeTrainingPlanAdaptationHistory(
    trainingPlanAdaptationHistory,
  );
  ...
  return {
    ...
    streakDays: getWorkoutStreakDays(productiveWorkouts, currentDateKey),
    ...
    scheduledWorkoutsCount: Array.isArray(scheduledWorkouts)
      ...
      : 0,
    statusBreakdown,
    loadTrend: buildLoadTrend(productiveWorkouts, currentDateKey),
